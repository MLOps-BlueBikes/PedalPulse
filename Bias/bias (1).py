...
bike_undocked = combined_df.groupby(['started_at', 'start_station_name']).size().reset_index(name='BikeUndocked')

# # Merge the result back into final_df
final_df = pd.merge(combined_df, bike_undocked, on=['started_at', 'start_station_name'], how='left')
...
columns_to_keep = ['started_at','start_station_name', 'start_station_id','month', 'hour', 'day_name','duration',
       'distance_km','Temperature (°F)','Humidity','Wind Speed','Precip.', 'Condition','BikeUndocked','member_casual','rideable_type']
...
most_freq_data = hourly_station_data[hourly_station_data['start_station_name'].isin(most_frequent_stations)]
least_freq_data = hourly_station_data[hourly_station_data['start_station_name'].isin(least_frequent_stations)]

# One-hot encode 'day_name' and 'Condition'
most_freq_encoded = pd.get_dummies(most_freq_data, columns=['day_name', 'Condition'], drop_first=True)
least_freq_encoded = pd.get_dummies(least_freq_data, columns=['day_name', 'Condition'], drop_first=True)
...
def prepare_data(data):
    ...
    X['start_station_name'] = X['start_station_name'].map(station_freq_map)  # Frequency encoding
...
X_train_most, X_test_most, y_train_most, y_test_most = train_test_split(X_most, y_most, test_size=0.2, random_state=42)
X_train_least, X_test_least, y_train_least, y_test_least = train_test_split(X_least, y_least, test_size=0.2, random_state=42)
...
train_and_log_model(X_train_most, X_test_most, y_train_most, y_test_most, "Most Frequent Stations")
train_and_log_model(X_train_least, X_test_least, y_train_least, y_test_least, "Least Frequent Stations")
...
required_columns = [
    'start_station_name', 'started_at', 'month', 'hour', 'day_name', 'duration',
    'distance_km', 'Temperature (°F)', 'Humidity', 'Wind Speed', 'Precip.',
    'Condition', 'BikeUndocked'
]
...
resampled_most_freq = resample_and_aggregate_by_station(most_freq_data, frequency='h')  # Hourly resampling
resampled_least_freq = resample_and_aggregate_by_station(least_freq_data, frequency='h')  # Hourly resampling

# One-hot encode `day_name` and `Condition`
resampled_most_encoded = pd.get_dummies(resampled_most_freq, columns=['day_name', 'Condition'], drop_first=True)
resampled_least_encoded = pd.get_dummies(resampled_least_freq, columns=['day_name', 'Condition'], drop_first=True)
...
X_train_resampled_most, X_test_resampled_most, y_train_resampled_most, y_test_resampled_most = train_test_split(
    X_resampled_most, y_resampled_most, test_size=0.2, random_state=42
)
X_train_resampled_least, X_test_resampled_least, y_train_resampled_least, y_test_resampled_least = train_test_split(
