...
data_cleaned = load_data_from_gcs(GCP_BUCKET_NAME, GCP_DATA_FILE)  # Replace with your actual file path
...
encoded_data = pd.get_dummies(hourly_station_data, columns=['day_name', 'Condition', 'rideable_type'], drop_first=True)
...
most_frequent_station = station_freq.idxmax()  # Get the station name with the maximum frequency
least_frequent_station = station_freq.idxmin()  # Get the station name with the minimum frequency
...
sns.barplot(x=['Overall', 'Most Frequent Station', 'Least Frequent Station', 'Classic Bike', 'E-Bike'],
