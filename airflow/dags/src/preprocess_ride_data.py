...
def remove_missing_values(df):
    task_logger.info("Removing missing values.")
    return df.dropna()


def remove_duplicates(df):
    task_logger.info("Removing duplicate rows.")
    return df.drop_duplicates()


# Data preprocessing helper functions
def data_type_conversion(df):
    task_logger.info("Converting data types.")
...
def extract_temporal_features(df):
    task_logger.info("Extracting temporal features.")
...
def clean_and_preprocess_data(filepath, chunk_size=50000, **context):
    ...
    if not os.path.exists('/opt/airflow/dags/data/preprocessed_tripdata/'):
        os.mkdir('/opt/airflow/dags/data/preprocessed_tripdata/')

    output_path = os.path.join('/opt/airflow/dags/data/preprocessed_tripdata/', "preprocessed-"+os.path.basename(filepath))

    task_logger.info(f"Starting data cleaning and preprocessing for file: {filepath}")

    # Check if the file exists
    if not os.path.exists(filepath):
        task_logger.error(f"File {filepath} does not exist. Skipping processing.")
        return AirflowSkipException

    # Remove the output file if it already exists to avoid appending to old data
    if os.path.exists(output_path):
        task_logger.warning(f"Output file {output_path} already exists. Hence, removing it....")
    ...
    for chunk in pd.read_csv(filepath, chunksize=chunk_size):
        try:
            ...
            task_logger.error(f"Error encountered during chunk processing for {filepath} : {e}")
            raise e
