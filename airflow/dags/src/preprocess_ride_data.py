...
def extract_temporal_features(df):
    ...
    df['distance_km'] = df.apply(lambda row: haversine_distance(row['start_lat'], row['start_lng'], row['end_lat'], row['end_lng']), axis=1)
    return df
...
def clean_and_preprocess_data(extract_dir, clean_dir, chunk_size=50000, **context):
    ...
    if os.path.exists(output_path):
        logging.warning(f"Output file {output_path} already exists. Removing it.")
    ...
    logging.info(f"Data cleaning and preprocessing completed for file: {filename}. Output saved to: {output_path}")
