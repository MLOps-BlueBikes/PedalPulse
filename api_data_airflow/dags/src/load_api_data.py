...
def load_api_data():
    ...
    try:
        ...
        if status_response.status_code == 200:
            ...
            logging.info(f"Succesfully downloaded station status data at: {status_output_path}")

        else:
            logging.error(f"Failed to retrieve data. Status code: {status_response.status_code}")
        ...
        if info_response.status_code == 200:
            ...
            logging.info(f"Succesfully downloaded station info data at: {info_output_path}")

        else:
            logging.error(f"Failed to retrieve data. Status code: {info_response.status_code}")
