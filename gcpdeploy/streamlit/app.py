...
if endpoint_id:
    ...
    with st.form(key="input_form"):
        start_station_name = st.number_input(
            "Start Station Name", min_value=0, max_value=100
        )
        ...
        temperature = st.number_input(
            "Temperature (°F)", min_value=-50.0, max_value=150.0
        )
        humidity = st.number_input("Humidity (%)", min_value=0.0, max_value=100.0)
        wind_speed = st.number_input(
            "Wind Speed (km/h)", min_value=0.0, max_value=150.0
        )
    ...
    if submit_button:
        all_days = ["Monday", "Saturday", "Sunday", "Thursday", "Tuesday", "Wednesday"]
        all_conditions = [
            "Cloudy / Windy",
            "Fair",
            "Fair / Windy",
        ...
        conditions = {
            f"Condition_{cond}": 1 if condition == cond else 0
        ...
        if response.predictions:  # Check if there are predictions
            predicted_bikes = round(
                response.predictions[0]
            )  # Extract and round the first prediction
