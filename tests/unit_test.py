...
def download_and_extract_data(logical_date):
    ...
    while True:
        ...
        try:
            ...
            print(f"Failed to download data for {logical_date.strftime('%Y-%m')}."
...
def test_missing_values(data):
    ...
    assert missing_count <= max_allowed_missing, (
...
def test_invalid_lat_long(data):
    ...
    assert ((data['end_lng'].dropna() >= -180) & (data['end_lng'].dropna() <= 180)).all()
